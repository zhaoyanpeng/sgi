...
# Toy COPY experiment w/ modeling relative relations. 
# bash bash/run_copy_rel.sh default 0 
...
model.relation.num_p=32 model.relation.cat_p=True model.relation.p_dim=512 
model.encoder.num_p=-1 model.encoder.w_dim=512 model.encoder.num_w=1
optimizer.lr=5e-5 optimizer.scheduler=[MultiStepLR,{milestones:[5,12,15,20],gamma:0.5}]
