...
dec_vocab_name="Sentences_1002_all.topk.dict"

# config for AbstractScene 1.1
data_root="$HOME/backup/data/scene/AbstractScenes_v1.1"
more_root=$data_root

vgg_name=vgg19_bn

# Train VC-MLM w/ visual object representations (encoded via pre-trained VGG)
# bash bash/run_scene_vis.sh default 0

# train: clevr 
model_name="sgi.thesis.vis.scene.test.$seed"

mtask="alias_name=$model_name
monitor=AbsceneMonitor
...
data.input_cap_type=part_caps

data.cate_type=\"\"
data.cate_max_len=1

data.name=abscene
data.load_obj=True data.more_root='\${.data_root}/object_$vgg_name'

data.mlm_prob=0.15
data.relation_words=[left,right,in_front_of,behind]
model.loss.alpha_l1=0.
model.loss.name=MLMLossHead
model.decoder.name=SGIMiniTFMLMDecHead
...
model.encoder.skip_embed=True
model.encoder.num_p=-4 model.encoder.cat_p=False model.encoder.p_dim=256 model.encoder.w_dim=1024 model.encoder.p_type=learned
...
running.epochs=2 running.batch_size=50 running.peep_rate=100
running.save_rate=1e9 running.save_epoch=True running.skip_save=False running.save_last=True


data.data_name=[0,2] data.eval_name=[2,4] data.test_name=[6,2] data.eval_samples=1e3
"
