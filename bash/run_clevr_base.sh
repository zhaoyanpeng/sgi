...
data.cate_max_len=40
data.cate_type=atomic_object
model.loss.optim_only_relation=True
model.decoder.num_p=512 
model.encoder.p_type=learned model.encoder.p_dim=512 model.encoder.cat_p=True 
optimizer.max_gnorm=1 optimizer.lr=5e-5 optimizer.scheduler=[MultiStepLR,{milestones:[15,36,45,50],gamma:0.5}]
...
running.epochs=200 running.batch_size=50 running.peep_rate=100 running.save_rate=1e9 running.save_epoch=True
running.skip_save=True
